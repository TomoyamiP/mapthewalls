...
export default function Archive() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 2) DEV ONLY: also load localStorage spots so local test uploads show up
        let mergedSpots: GraffitiSpot[] = supabaseSpots;
        if (import.meta.env.DEV) {
          const { loadSpots } = await import("../lib/storage");
          const localSpots = loadSpots();

          // Merge by id (Supabase wins)
          const byId = new Map<string, GraffitiSpot>();
          for (const s of supabaseSpots) byId.set(s.id, s);
          for (const s of localSpots) if (!byId.has(s.id)) byId.set(s.id, s);

          mergedSpots = Array.from(byId.values());
        }

        // 3) Load votes for Supabase spots only (local-only spots won't have votes)
        ...
        if (error) {
          ...
          // still show the list (with merged dev spots) even if votes fail
          setSpots(mergedSpots);
          return;
        }
        ...
        const withAvg = mergedSpots.map((s: any) => {
  ...
  const sortedSpots = useMemo(() => {
    ...
    switch (sortKey) {
      ...
      case "rating_asc":
        return spotsCopy.sort(
          (a: any, b: any) => (a.avgRating ?? 0) - (b.avgRating ?? 0)
        );
  ...
  return (
    <div className="pt-[calc(5.5rem+env(safe-area-inset-top))] min-h-screen bg-zinc-950 text-zinc-100">
